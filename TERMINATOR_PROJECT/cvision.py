...
def create_detector(dictionary):
    ...
    # Set the expected size range (in pixels) for marker recognition
...
#                           (empty array of if none detected, array of arrays if several detected)
...
class Obj:
    ...
    # Calculate the orientation of the ArUco marker in the image from the position of the 4 corners.
...
class Map:
    def __init__(self, ref1, ref2, ref3, ref4, distance_r1r2, distance_r3r2):
        self.ref1 = ref1
        self.ref1_map_pos = np.array([1, 0])
        self.ref2 = ref2
        self.ref2_map_pos = np.array([0, 0])
        self.ref3 = ref3
        self.ref3_map_pos = np.array([0, 1])
        self.ref4 = ref4
        self.ref4_map_pos = np.array([1, 1])
        self.origin = np.array([0, 0])
        self.distance_r1r2 = distance_r1r2  #[mm]
        self.distance_r3r2 = distance_r3r2  #[mm]
        self.e1 = (self.ref1_map_pos - self.ref2_map_pos)/np.linalg.norm(self.ref1_map_pos - self.ref2_map_pos)
        self.e2 = (self.ref3_map_pos - self.ref2_map_pos)/np.linalg.norm(self.ref3_map_pos - self.ref2_map_pos)
        self.scale = np.array([distance_r1r2/np.linalg.norm(self.ref1_map_pos - self.ref2_map_pos), distance_r3r2/np.linalg.norm(self.ref3_map_pos - self.ref2_map_pos)])
    
    # Check that all the three references needed to use the map are detected.
    def references_detected(self):
        if (np.size(self.ref1.img_pos,0) > 0) and (np.size(self.ref2.img_pos,0) > 0) and (np.size(self.ref3.img_pos,0) > 0) and (np.size(self.ref4.img_pos,0) > 0):
            return True
        else:
            return False
    ...
    def matrix(self, frame, robot, goal):
        ...
        square_size = 10 #[mm]
        margins = 0 # Nb of cells around the references of the map
        origin = np.array([margins, margins]) # Position of the origin of the map in the matrix
        ...
        mask = cv2.inRange(frame, lower_b, upper_b)
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # Find the contours of the obstacles in the binary mask       
        obstacle_img = np.multiply(np.ones_like(frame),255) # Create an empty image to store the contours of the obstacles
        for contour in contours:
            area = cv2.contourArea(contour)
            if area > 50:
                # Draw the contours with an area > 50 pixels on the image
                cv2.drawContours(obstacle_img, [contour], -1, 0, thickness=cv2.FILLED)
        show_frame(obstacle_img)
        max_y, max_x, _ = obstacle_img.shape
        for x in range(2*margins+self.distance_r1r2//square_size):
            for y in range(2*margins+self.distance_r3r2//square_size):
                ...
                if max_x > loc_img[0] and max_y > loc_img[1]:
                    if obstacle_img[loc_img[1], loc_img[0]].all() < 1:
                        matrix[x,y] = 1
        ...
        if np.size(map_pos,0) > 0:
            if not(np.isnan(map_pos[0]).any()):
                mat_pos = origin + np.round(np.divide(map_pos[0],square_size)).astype(int) # Only one robot is drawn on the matrix
        ...
        if np.size(map_pos,0) > 0:
            if not(np.isnan(map_pos[0]).any()):
                mat_pos = origin + np.round(np.divide(map_pos[0],square_size)).astype(int) # Only one goal is drawn on the matrix
                if mat_pos[0] > 0 and mat_pos[1] > 0 and mat_pos[0] < np.size(matrix,0) and mat_pos[1] < np.size(matrix,1):
                    matrix[mat_pos[0],mat_pos[1]] = 3
        return matrix

    # Correct the deformation due to the angle of view of the camera. Enable to use the image from the camera more precisely.
    # Update the image positions and corners of the objects and return a corrected image.
    def corrected_view(self, frame, objects):
        # Build the transformation matrix to correct the image deformation 
        # from the known positions of the references on the map and the image.
        distorted_points = np.float32([self.ref1.img_pos[0], self.ref2.img_pos[0], self.ref3.img_pos[0], self.ref4.img_pos[0]])
        margin = 40 #[pixels]
        h,w = frame.shape[:2]
        if ((self.distance_r1r2/self.distance_r3r2) > (w/h)):
            x_axis = w-2*margin
            y_axis = self.distance_r3r2/self.distance_r1r2*x_axis
        else:
            y_axis = h-2*margin
            x_axis = self.distance_r1r2/self.distance_r3r2*y_axis
        undistorted_points = np.float32([[margin+x_axis,margin],[margin,margin],[margin,margin+y_axis],[margin+x_axis,margin+y_axis]])
        transform_mat = cv2.getPerspectiveTransform(distorted_points, undistorted_points)
        corrected_frame = cv2.warpPerspective(frame, transform_mat, (w,h), flags=cv2.INTER_LINEAR)
        # Apply the correction on the centers and corners of the detected ArUco markers.        
        for o in range(np.size(objects,0)):
            for i in range(np.size(objects[o].img_pos,0)):
                original_center = objects[o].img_pos[i]
                original_corners = objects[o].corners[i]
                # Convert the array of points to homogeneous coordinates
                original_center_homogeneous = np.array([original_center[0], original_center[1], 1])
                original_corners_homogeneous = np.column_stack((original_corners, np.ones(np.size(original_corners,0))))
                # Apply the perspective transformation to all points simultaneously
                transformed_center_homogeneous = np.dot(transform_mat, original_center_homogeneous.T).T
                transformed_corners_homogeneous = np.dot(transform_mat, original_corners_homogeneous.T).T
                # Convert the results back to Cartesian coordinates
                transformed_center = (transformed_center_homogeneous[0] / transformed_center_homogeneous[2], transformed_center_homogeneous[1] / transformed_center_homogeneous[2])
                transformed_corners = transformed_corners_homogeneous[:, :2] / transformed_corners_homogeneous[:, 2:]
                # Update with the corrected image points
                objects[o].img_pos[i] = transformed_center
                objects[o].corners[i] = transformed_corners
