...
def conversion(matrix):
    ...
    start_arr = np.where(arr == 2)
    goal_arr = np.where(arr == 3)
...
def grow_obstacles(matrix, growth_size):
    ...
    for i, j in zip(obstacle_indices[0], obstacle_indices[1]):
        ...
        row_range = slice(max(0, i - size), min(arr.shape[0], i + growth_size + 1))
        col_range = slice(max(0, j - size), min(arr.shape[1], j + growth_size + 1))
